...
14. QR Code e Code 128 identificam o registro; o QR aponta à verificação HTTPS. O fluxo legado de folha separada codifica `MAI|<id>|R<versão>`; o provider privado com evidência incorporada codifica `<id>|<número-do-documento>`. Nenhum deles substitui o hash ou a assinatura PAdES.
...
A folha final é A4, uma página, e mantém todo o grid dentro de 3 cm no topo e à esquerda e 2 cm à direita e na base. Ela contém ID textual, número documental, total de páginas, hash SHA-256 da entrada, QR com correção de erro H, Code 128, endereços de validação, metadados, atributos, atestado ML-DSA-65 e evidência visual do signatário. O fundo de segurança não tem moldura perimetral e atravessa toda a página; os grupos não recebem contornos. Véus brancos translúcidos, acentos estreitos e espaço negativo organizam a informação como uma única impressão. O QR preserva uma área silenciosa clara sem traço externo. Todas as páginas recebem linha dourada superior de `3 pt`. As páginas de conteúdo também recebem uma faixa lateral direita sem divisor, com `m.` centralizado e uma única inscrição contínua, sem quebra: `ASSINATURA.MAIOCCHI.ADV.BR - DOCUMENTO <número> - HASH <SHA-256> - CÓDIGO <PQC-MLDSA65> - VERIFICAÇÃO <ID público> - PÁG <atual> DE <total>`; a folha final não repete essa faixa. A paginação física aparece somente no registro lateral, sem rodapé autônomo; a contagem total também permanece nos metadados documentais. O texto canônico é:
